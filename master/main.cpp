...
volatile uint8_t runningSonar_Global = 'F'; //F - false - trwanie jakiego� pomiaru
volatile uint8_t echoRising_Global = false;
//volatile uint8_t configRef_Global = false; //przy konfiguracji czujnika musi by� true
...
uint8_t relayTime_Global = 5; // [min]
uint8_t ledTimeOn_Global = 8; // [sec]
uint8_t ledIncrease = 30;//0   [ms]
class ledBrightConfig{ //TODO: ogarna� t� klase i to r�wnaniedo czujnika �wiat�a    (valueMax - valueMin) * actualFromADC
...
ledBrightConfig ledBrightConfig(100, 100, 100, 255);
...
void sonar_config(void)
{
	...
	while(runningSonar_Global != 'F') asm("NOP"::); //czekamy na skonczenie si� pomiaru
...
void port_init(void) //inicjalizacja port�w 
{
...
{
	MCUCR |= (1<<ISC10)|(1<<ISC00); //wywo�ywanie przerwania przy kia�dym zboczu - zewn�trzne przerwanie int0
...
void start_lighting(position goradol)
{
	...
	{
		OCR1A = 0;
		OCR1B = (16000 * ledBrightConfig.ledBright)/100; //16 ty�, max warto��
...
ISR(INT1_vect) //CZUJNIK NR 1
{
	if (runningSonar_Global == 1)
	{
		if(echoRising_Global){
			...
			echoRising_Global  = 0; //czekamy na nast�pny stan - to b�dzie opadajacy 			
		}else {
...
ISR(TIMER0_OVF_vect) //licznik czasu impuls�w z czujnika 0 lub 1
{
	timerCounter_Global++;
