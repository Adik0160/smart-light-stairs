...
//	configRef_Global = configRef;//configRef = 1 kiedy w³¹czamy konfiguracje czujnika - 0 przy zwyk³ym skanowanius
	...
	while((runningSonar_Global != 'F') && waitingFlag) asm("NOP"::);
...
{
	while((runningSonar_Global != 'F') && waitingFlag) asm("NOP"::);  //je¿eli trwa jakiœ pomiar - czekaj
	measure();
	while((runningSonar_Global != 'F') && waitingFlag) asm("NOP"::); //czekaj na skoñczenie pomiaru, póŸniej porównuj
